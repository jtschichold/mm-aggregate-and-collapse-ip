...
function parseInputs(): ActionInputs {
    ...
    if (filterReservedIPs && filterReservedIPs.toUpperCase() !== 'FALSE') result.filterReservedIPs = true

    const minIPv6Mask: string = core.getInput('minIPv6Mask')
    if (minIPv6Mask) result.filterOptions.minV6SubnetMask = parseInt(minIPv6Mask)

    const minIPv4Mask: string = core.getInput('minIPv4Mask')
    if (minIPv4Mask) result.filterOptions.minV4SubnetMask = parseInt(minIPv4Mask)
...
async function run(): Promise<void> {
    try {
        ...
        let initialList: iplist.IPNetwork[] = []
        ...
        if (filterListV4.length !== 0 || filterListV6.length !== 0 || inputs.filterOptions.minV4SubnetMask !== 0 || inputs.filterOptions.minV6SubnetMask !== 0) {
            ({result, delta} = iplist.filter(result, filterListV4.concat(filterListV6), inputs.filterOptions))
        }

        // save my stuff
        let deltaPath = tmp.tmpNameSync({template: inputs.tmpTemplate, tmpdir: '.'})
        await iplist.write(deltaPath, delta)

        let resultPath = tmp.tmpNameSync({template: inputs.tmpTemplate, tmpdir: '.'})
